...
def gen_positive_instances(data_root, prepare_root, split='train'):
    ...
    rlt_path = os.path.join(data_root, '%s_2019.json' % split)
    ...
    print('bad hoi count: %d' % bad_hoi_cnt)
    print('pos hoi count: %d' % pos_hoi_cnt)
    rlt_anno_hico_path = os.path.join(prepare_root, '%s_GT_PIC.pkl' % split)
    with open(rlt_anno_hico_path, 'wb') as f:
        # rlt_anno_hico = {'trainval_017802': rlt_anno_hico['trainval_017802']}
...
def gen_negative_instances(data_root, prepare_root, split='train'):
    ...
    # collect object instances
    org_rlt_path = os.path.join(data_root, '%s_2019.json' % split)
    ...
    with open(neg_rlt_anno_hico_path, 'wb') as f:
        ...
        # neg_rlt_anno_hico = {'trainval_017802': neg_rlt_anno_hico['trainval_017802']}
