...
class VCOCOeval(object):
    ...
    def _do_agent_eval(self, vcocodb, detections_file, ovr_thresh=0.5):
        ...
        for i in range(len(vcocodb)):
            ...
            for aid in range(self.num_actions):
                ...
                covered = np.zeros((gt_boxes.shape[0]),
                                   dtype=np.bool)  # gt_boxes.shape[0] is the number of people in this image
                ...
                for j in idx:  # Each predicted person
                    ...
                    overlaps = get_overlap(gt_boxes,
                                           pred_box)  # overlap between this predict human and all human gt_boxes
                    ...
                    is_true_action = (gt_actions[
                                          jmax, aid] == 1)  # Is this person actually doing this action according to gt?

                    sc[aid].append(
                        agent_scores[j])  # The predicted score of this person doing this action. In descending order.
                    if is_true_action and (
                            ovmax >= ovr_thresh):  # bounding box IOU is larger than 0.5 and this this person is doing this action.
                        if covered[jmax]:
                            fp[aid].append(1)
                            tp[aid].append(0)
        ...
        for aid in range(self.num_actions):
            info = '{: >20}: AP = {:0.2f} (#pos = {:d})'.format(self.actions[aid], agent_ap[aid] * 100.0,
                                                                int(npos[aid]))
            print(info)
            f.write(info)
            f.write('\n')
