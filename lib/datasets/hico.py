...
class hico(imdb):
    ...
    def __init__(self, image_set, version):
        imdb.__init__(self, 'hico_' + version + '_' + image_set)
    ...
    def _to_multilabel_instances(self, gt_roidb):
        ...
        for image_anno in gt_roidb:
            ...
            object_classes = image_anno['obj_classes']
            hoi_classes = image_anno['hoi_classes']

            num_hoi = hboxes.shape[0]
            for i in range(num_hoi):
                hbox1 = hboxes[i]
                obox1 = oboxes[i]
                obj_id1 = object_classes[i]

                for j in range(i+1, num_hoi):
                    hbox2 = hboxes[j]
                    obox2 = oboxes[j]
                    obj_id2 = object_classes[j]
                    if obj_id1 == obj_id2 and iou(hbox1, hbox2) > ov_thr and iou(obox1, obox2) > ov_thr:
                        hoi_classes[i] = np.logical_or(hoi_classes[i], hoi_classes[j])
                        hoi_classes[j] = np.logical_or(hoi_classes[i], hoi_classes[j])
    ...
    def _load_all_annotations(self):
        ...
        for image_id, mat_anno in enumerate(mat_anno_db[0, :]):
            ...
            for mat_hoi in mat_hois:
                ...
                obj_class_id = self.object_class2ind[hoi_class.object_name()]
