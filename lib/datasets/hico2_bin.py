...
class hico2_bin(imdb):
    ...
    def gt_roidb(self):
        cache_file = os.path.join(self.cache_path, self.name + '_gt_roidb_bin.pkl')
    ...
    def _load_all_annotations(self):
        ...
        for image_id, img_pos_hois in anno_gt_db.items():
            ...
            if image_id in anno_ng_db and len(anno_ng_db[image_id]) > 0:
                ...
                if len(img_neg_hois0) > 3 * len(img_pos_hois):
                    inds = random.sample(range(len(img_neg_hois0)), 3 * len(img_pos_hois))
                else:
                    inds = []
                    for i in range(int(3 * len(img_pos_hois) / len(img_neg_hois0))):
                        inds += range(len(img_neg_hois0))
                    for i in range(3 * len(img_pos_hois) - len(inds)):
                        inds.append(i)
                img_neg_hois = [img_neg_hois0[ind] for ind in inds]
                assert len(img_neg_hois) == (3 * len(img_pos_hois))
            else:
                img_neg_hois = []
