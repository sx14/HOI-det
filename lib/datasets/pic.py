...
class pic(imdb):
    ...
    def load_hoi_classes(data_path):
        ...
        import json
        with open(os.path.join(data_path, 'categories_list', 'label_categories.json'), 'r') as f:
            obj_cls_list = json.load(f)
            obj_cls_list = sorted(obj_cls_list, key=lambda item: item['id'])
            obj_cls_list = [item['name'] for item in obj_cls_list]
            obj_cls_to_ind = {name: i for i, name in enumerate(obj_cls_list)}

        with open(os.path.join(data_path, 'categories_list', 'relation_categories.json'), 'r') as f:
            vrb_cls_list = json.load(f)
            vrb_cls_list = sorted(vrb_cls_list, key=lambda item: item['id'])
            vrb_cls_list = [item['name'] for item in vrb_cls_list]
        ...
        return obj_cls_list, vrb_cls_list, obj_cls_to_ind, vrb_cls_to_ind, obj_id_to_vrb_ids

    def __init__(self, image_set, version):
        imdb.__init__(self, 'pic_' + version + '_' + image_set)
        ...
        self.obj_id_to_vrb_ids = self.load_hoi_classes(self._data_path)
    ...
    def image_path_from_index(self, index):
        ...
        """
        index = index.split('&')[0]
        image_path = os.path.join(self._data_path, 'image', self._image_set,
    ...
    def _load_image_set_info(self):
        ...
        if os.path.exists(all_image_info_path):
            ...
            all_image_info = {}
            image_root = os.path.join(self._data_path, 'image', self._image_set)
    ...
    def _get_default_path(self):
        return os.path.join(cfg.DATA_DIR, 'pic')
    ...
    def augment_hoi_instances(self, raw_hois, im_hw):
        ...
        for raw_hoi in raw_hois:
            ...
            hbox = raw_hoi[2]
            obox = raw_hoi[3]
            aug_hboxes = self.augment_box(hbox, im_hw, 2)
            aug_oboxes = self.augment_box(obox, im_hw, 2)
        ...
        for image_id, img_pos_hois in anno_gt_db.items():
            ...
            for pn, hois in enumerate([img_pos_hois, img_neg_hois]):
                for raw_hoi in hois:
                    ...
                    image_anno['vrb_masks'].append(self.obj_id_to_vrb_ids[obj_class_id])
