...
class hoia(imdb):
    ...
    def load_obj2vec(data_path):
        obj2vec_path = os.path.join(data_path, 'mlcnet_data', 'obj2vec.pkl')
    ...
    def load_hoi_classes(data_path):
        ...
        import json
        with open(os.path.join(data_path, 'object.txt'), 'r') as f:
            obj_cls_list = f.readlines()
            obj_cls_list = [line.strip() for line in obj_cls_list]
            obj_cls_list = ['none'] + obj_cls_list
            obj_cls_to_ind = {name: i for i, name in enumerate(obj_cls_list)}

        with open(os.path.join(data_path, 'relation.txt'), 'r') as f:
            vrb_cls_list = f.readlines()
            vrb_cls_list = [line.strip() for line in vrb_cls_list]
            vrb_cls_list = ['none'] + vrb_cls_list
            vrb_cls_to_ind = {name: i for i, name in enumerate(vrb_cls_list)}

        # with open(os.path.join(data_path, 'mlcnet_data', 'object_to_relations.json'), 'r') as f:
        #     obj_id_to_vrb_ids = json.load(f)
        #     obj_id_to_vrb_ids = {int(item[0]): item[1] for item in obj_id_to_vrb_ids.items()}

        with open(os.path.join(data_path, 'mlcnet_data', 'prior.pkl'), 'rb') as f:
            obj_to_pre_dist = pickle.load(f)
        obj_to_pre_mask = obj_to_pre_dist > 0.01
        obj_to_pre_mask = obj_to_pre_mask.astype('float32')
        obj_to_pre_mask[:, 0] = 1.0
        return obj_cls_list, vrb_cls_list, obj_cls_to_ind, vrb_cls_to_ind, obj_to_pre_mask

    def __init__(self, image_set, version):
        imdb.__init__(self, 'hoia_' + version + '_' + image_set)
        self._version = version
        self._image_set = image_set
        self._data_path = self._get_default_path()

        self.obj_classes, self.vrb_classes, \
        self.obj_class2ind, self.verb_class2ind, \
        self.obj_to_pre_mask = self.load_hoi_classes(self._data_path)
        self._classes = self.vrb_classes
        self._class_to_ind = dict(zip(self._classes, xrange(len(self._classes))))

        self._image_exts = ['png', 'PNG', 'jpg', 'JPG', 'jpeg', 'JPEG']
        self._depth_ext = '.npy'
    ...
    def obj2vec(self):
        if self._obj2vec is None:
            obj2vec_path = os.path.join(self._data_path, 'mlcnet_data', 'obj2vec.pkl')
            with open(obj2vec_path, 'rb') as f:
                obj2vec = pickle.load(f, encoding='latin1')
    ...
    def image_path_from_index(self, index):
        ...
        for ext in self._image_exts:
            ...
            if os.path.exists(image_path):
                return image_path
        print('unknown image ext: %s' % index)
        return None
    ...
    def _load_image_set_info(self):
        ...
        all_image_info_path = os.path.join(self._data_path, 'mlcnet_data', 'all_image_info.pkl')
        if os.path.exists(all_image_info_path):
            with open(all_image_info_path, 'rb') as f:
            ...
            with open(all_image_info_path, 'wb') as f:
                pickle.dump(all_image_info, f)
            print('all_image_info saved at: %s' % all_image_info_path)
        return all_image_info

    def _get_default_path(self):
        return os.path.join(cfg.DATA_DIR, 'hoia')
    ...
    def augment_hoi_instances(self, raw_hois, im_hw):
        ...
        for raw_hoi in raw_hois:
            ...
            hbox = raw_hoi[2]
            obox = raw_hoi[3]
            aug_hboxes = self.augment_box(hbox, im_hw, 5)
            aug_oboxes = self.augment_box(obox, im_hw, 5)
            ...
            for i in range(aug_hboxes.shape[0]):
                ...
                new_hois.append([raw_hoi[0],             # stub
                                 raw_hoi[1],
                                 aug_hbox,
                                 aug_obox,
                                 raw_hoi[4],             # stub
    ...
    def _load_all_annotations(self):
        ...
        anno_ng_db = pickle.load(open(os.path.join(self._data_path, 'mlcnet_data', '%s_NG_PIC.pkl' % self._image_set), 'rb'), encoding='latin1')
        anno_gt_db = pickle.load(open(os.path.join(self._data_path, 'mlcnet_data', '%s_GT_PIC.pkl' % self._image_set), 'rb'), encoding='latin1')
        ...
        for image_id, img_pos_hois in anno_gt_db.items():
            ...
            if image_id not in self._all_image_info:
                continue

            # augment positive instances
            im_w = self._all_image_info[image_id][0]
            im_h = self._all_image_info[image_id][1]
            image_hw = [im_h, im_w]
            ...
            image_anno = {'hboxes': [],
                          ...
                          'hoi_masks': [],
                          'vrb_masks': [],
                          'width': self._all_image_info[image_id][0],
                          'height': self._all_image_info[image_id][1],
                          'flipped': False}
            # print('pos-neg: %d-%d' % (len(img_pos_hois), len(img_neg_hois)))
            for pn, hois in enumerate([img_pos_hois, img_neg_hois]):
                for raw_hoi in hois:

                    obj_class_id = raw_hoi[4]
                    vrb_class_ids = raw_hoi[1]

                    # hoi_class_ids = raw_hoi[1]
                    # obj_class_id = hoi_class_ids[0][1]
                    # vrb_class_ids = [hoi_class[0] for hoi_class in hoi_class_ids]
                    ...
                    image_anno['vrb_masks'].append(self.obj_to_pre_mask[obj_class_id])
            ...
            if len(image_anno['hboxes']) == 0:
                ...
                image_anno['hoi_classes'] = np.zeros((0, 1))
                image_anno['vrb_classes'] = np.zeros((0, len(self.vrb_classes)))
                image_anno['hoi_masks'] = np.ones((0, 1))
                ...
                image_anno['pbox_lists'] = np.zeros((len(image_anno['hboxes']), 6, 5))
                image_anno['pbox_lists1'] = np.zeros((len(image_anno['hboxes']), 6, 5))
                ...
                image_anno['hoi_classes'] = np.zeros((len(image_anno['hboxes']), 1))
                ...
                image_anno['hoi_masks'] = np.zeros((len(image_anno['hboxes']), 1))
                ...
                for i, vrb_mask in enumerate(vrb_masks):
                    image_anno['vrb_masks'][i] = vrb_mask
            ...
            if ins_num > batch_size:
                ...
                for b in range(int(ins_num / batch_size)):
                    ...
                    all_annos[image_id+'&%d' % b] = image_anno1
            else:
                all_annos[image_id] = image_anno
    ...
    def _get_widths(self):
        ...
        for image_id in self._image_index:
            all_widths.append(self._all_image_info[image_id][0])
        return all_widths
