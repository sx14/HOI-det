...
class resnet(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.iRCNN_top_att = nn.Conv2d(2048, 1, (1, 1))
    self.hRCNN_top_att = nn.Conv2d(2048, 1, (1, 1))
    self.oRCNN_top_att = nn.Conv2d(2048, 1, (1, 1))
  ...
  def _ihead_to_tail(self, pool5):
    ...
    att = self.iRCNN_top_att(pool5)   # 7x7x1
    fc7 = (pool5 * att + pool5).mean(3).mean(2)
    return fc7

  def _hhead_to_tail(self, pool5):
    pool5 = self.hRCNN_top(pool5)     # 7x7x2048
    att = self.hRCNN_top_att(pool5)   # 7x7x1
    fc7 = (pool5 * att + pool5).mean(3).mean(2)
    return fc7

  def _ohead_to_tail(self, pool5):
    pool5 = self.oRCNN_top(pool5)     # 7x7x2048
    att = self.oRCNN_top_att(pool5)   # 7x7x1
    fc7 = (pool5 * att + pool5).mean(3).mean(2)
    return fc7
